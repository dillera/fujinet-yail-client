#include "vbxe.h"
#include "xdlc.h"
#include "utility.h"
#include "settings.h"
#include "graphics.h"

#include <conio.h>

#include <atari.h>
#include <string.h>
#include <stdbool.h>

extern Settings settings;
extern byte framebuffer[];

struct __vbxe* VBXE = NULL; // Empty if there is no VBXE present

void detect_VBXE(void)
{
    if (VBXE) return; // Already detected

    // The memory window to the VBXE
    if(VBXE_D640.CORE_VERSION != 0x10 && (VBXE_D640.MINOR_BERSION & 0x70) != 0x20) {
        // Try the second address
        if(VBXE_D740.CORE_VERSION != 0x10 && (VBXE_D740.MINOR_BERSION & 0x70) != 0x20) {
            // show_error("VBXE not detected\n\r"); // Don't show error on auto-detect
            return;
        }
        else
            VBXE = &VBXE_D740;
    }
    else
        VBXE = &VBXE_D640;

    cprintf("VBXE DETECTED\n\r");
}

void setup_VBXE(void)
{
    if (!VBXE)
        detect_VBXE();

    if (!VBXE)
    {
        show_error("VBXE not detected\n\r");
        return;
    }

    //cprintf("VBXE detected, %04X\n\r", VBXE);

    VBXE->VIDEO_CONTROL = 0x82;    // No-trans and 

    // Set memory window location to the same location as our "Frame Buffer".
    VBXE->MEM_BANK_SEL = (uint8_t)((uint16_t)&framebuffer[0] >> 1); //0x60;
    VBXE->MEMAC_CTRL = 0x88; // let only the CPU have access

    // XDL ADDRESS, this is placed after the image on a 4K boundary ($1000)
    // The image will take up 320x240 (76800, $12C00) bytes.  This is 19 banks (@ $4096 per bank),
    // so the XDL will be placed at $15000.
    VBXE->XDL_ADR0 = 0x00;
    VBXE->XDL_ADR1 = 0x50;
    VBXE->XDL_ADR2 = 0x01;

    VBXE->MEM_BANK_SEL = 128 + 21;

    // Setup XDL
    // XDL Structure: [Instruction 2 bytes] [Address 3] [Step 2] [Attribute 2] [Repeat 1]
    
    // Instruction
    XDL[0] = XDLC_GMON | XDLC_RPTL | XDLC_OVADR; 
    XDL[1] = XDLC_ATT | XDLC_END; 

    // Overlay location (Address) - XDL[2,3,4]
    XDL[2] = 0x00;
    XDL[3] = 0x00;
    XDL[4] = 0x00;

    // Step - XDL[5,6]
    XDL[5] = 0x40; // 0x40
    XDL[6] = 0x01; // 0x01 -> 0x0140 = 320

    // Attribute - XDL[7,8]
    XDL[7] = XDLC_ATT_OV_WIDTH_NORMAL | XDLC_ATT_OV_PALETTE_01; 
    XDL[8] = XDLC_ATT_MAIN_PRIORITY_OVERLAY_ALL ^ XDLC_ATT_MAIN_PRIORITY_OVERLAY_PF1; 

    // Repeat - XDL[9]
    XDL[9] = 239; // Repeat 239 lines

    // XDL disable
    VBXE->VIDEO_CONTROL = 0x00;

    // Clear the screen
    clear_vbxe();

    // Disable ANTIC
    //OS.sdmctl &= ~0x20;
}

void clear_vbxe() {
    int i;

    // Clear the screen
    for(i = 0; i < 19; ++i) {
        VBXE->MEM_BANK_SEL = 128 + i;  // MOVE WINDOW STARTING $0000
        memset(XDL, 0, 4096);
    }    
}